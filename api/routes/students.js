  ...
  req.getConnection(function(error, conn) {
    conn.query('SELECT * FROM student WHERE student_id = ' + req.params.student_id, function(err, rows, fields) {
      ...
      if (rows.length <= 0) {
        ...
        console.log("New get request for id:", req.params.student_id, "served")
        var score = 0;
        var scores = [];
        for (var item in JSON.parse(rows[0].scores)) {
          score += JSON.parse(rows[0].scores)[item];
        }
        console.log(scores)
        score = Math.floor(score / JSON.parse(rows[0].scores).length) + "%";
 ...
 */
app.post('/add/(:student_id)', function(req, res, next) {
  console.log("Deleted user", req.params.student_id)
  req.getConnection(function(error, conn) {
    conn.query('UPDATE student SET ? WHERE student_id = ' + req.params.student_id, student + req.params.student_id, function(err, result) {
      //if(err) throw err
      if (err) {
        req.flash('error', err)
...
// DELETE USER
app.get('/delete/(:student_id)', function(req, res, next) {
  console.log("Deleted user", req.params.student_id)
  req.getConnection(function(error, conn) {
